...
运行在飞牛 fnOS 上的 QQ 音乐下载器：扫码/验证码登录后，浏览推荐与私有歌单、搜索单曲，边听边看同步歌词，并把音频与元数据下载到 NAS 指定目录。
...
| 下载 | 串行队列，音频与元数据双进度条；支持暂停/继续/重试/清空 |
| 元数据 | 歌名、歌手、专辑、封面、歌词（LRC）、翻译歌词写入音频文件 |
| 历史 | 下载历史（成功/失败原因），仅显示可访问的记录 |
| 设置 | 下载目录 / 文件夹访问权限（合并为一处：点「选择文件夹…」走 fnOS 内嵌选择器，选中即授权；下拉可切换已授权目录）、默认音质（FLAC / 320K / 128K）、歌词翻译开关、请求间隔 |

## 安装

1. 打开飞牛 fnOS「应用中心 → 手动安装」，选择 `qqmusic-downloader.fpk`。
2. 安装向导中填写默认音质与歌词翻译开关（安装向导没有文件夹选择器，下载目录改在应用内点选）。
...
- 下载的是加密音频与元数据，需自行确认版权与使用范围，仅限个人合法使用。
